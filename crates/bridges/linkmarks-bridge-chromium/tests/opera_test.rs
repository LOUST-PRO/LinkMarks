...
fn parses_opera_gx_real_bookmarks() {
    ...
    let (bookmarks, errors) =
        linkmarks_bridge_chromium::parser::parse_and_flatten(path).expect("parse should succeed");
    eprintln!(
        "OPERA-GX: bookmarks={} errors={}",
        bookmarks.len(),
        errors.len()
    );
    for e in &errors {
    ...
    assert!(
        !bookmarks.is_empty(),
        "opera-gx should produce at least 1 bookmark"
    );
}
